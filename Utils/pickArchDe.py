...
class DiffEvolPickArch:
    ...
    def __init__(self, train_dict, param_pos_vals, max_iter,
        ...
        if not de_loss_dict:
            ...
            self.de_loss = {
                            'Res_Loss': tf.keras.losses.MeanSquaredError(),
        ...
        self.log_file_path = f"{log_dir}{self.name}_{self.session_ID}.csv"
        dat_frame.to_csv(self.log_file_path, mode='w')
    ...
    def _aux_min_func(self, arch_param_vec):
        ...
        fill_str = '-' * 50
        print('\n' + fill_str + f' {self.name}:: Averaging over training for {arch_str} ' + fill_str)
        ...
        for try_nb in range(self.avg_nb):
            ...
            train_hist = net_inst.train_net(nb_epochs=self.epochs, mb_size=self.mb_size)
            # ---------- Get and return the DE evolution loss ----------
            res_loss_part, param_loss, loss_dict = self.get_de_loss(net_inst.model, train_hist)
...
def main_test():
    ...
    de_inst = DiffEvolPickArch(train_dict, param_pos_vals, max_iter,
                               nb_epochs=nb_epochs, verbose=1, avg_nb=2)
