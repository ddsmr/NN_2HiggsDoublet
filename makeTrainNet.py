...
class PlotEndEpoch(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch % self.every_n_epoch == 0:
            ...
            if bool(self.cust_plot_fct):
                self.cust_plot_fct(test_arr, pred_arr, self.model_name, aux_mod_str=str(epoch + 1))
...
def _main_test():
    ...
    def cust_plot(test_arr, pred_arr, mod_str, aux_mod_str):
        """
    ...
    for _ in range(100):
        ...
        print(net_inst)
        hist_dict = net_inst.train_net(nb_epochs=10, mb_size=128, save_model=False, show_train_plot=False,
                                       plot_every_n=10
                                       )
        # del net_inst
